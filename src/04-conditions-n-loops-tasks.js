...
function doRectanglesOverlap(rect1, rect2) {
  const first = rect1.top + rect1.height;
  const second = rect2.top + rect2.left;
...
function findFirstSingleChar(str) {
  ...
  for (let i = 0; i < arr.length; i += 1) {
    const condition = arr.indexOf(arr[i]) === i && arr.indexOf(arr[i], i + 1) === -1;
...
function isCreditCardNumber(ccn) {
  ...
  let i = arr.length % 2 === 0 ? 0 : 1;

  for (i; i < arr.length; i += 2) {
    arr[i] = (arr[i] * 2) > 9 ? (arr[i] * 2 - 9) : (arr[i] * 2);
  }

  const sum = arr.reduce((a, c) => Number(a) + Number(c), 0);
...
function getCommonDirectoryPath(pathes) {
  ...
  const turnedArr = arr[0].map((_, i) => arr.map((e) => e[i]));

  const result = [];

  for (let i = 0; i < turnedArr.length; i += 1) {
    if (turnedArr[i].every((e) => e === turnedArr[i][0])) {
      result.push(turnedArr[i][0]);
    }
  }
...
function evaluateTicTacToePosition(pos) {
  for (let i = 0; i < pos.length; i += 1) {
    const condition = pos[i][0] === pos[i][1] && pos[i][1] === pos[i][2] && pos[i][0] !== undefined;
